                ...
                for x in list_downloads:
                    ...
                    for s in p_files:
                        ...
                        if s[-3::] == 'xml':
                            ...
                            # tag PublishDate - start
                            items = pdoc.getElementsByTagName(p_ntag + 'ns2:publicationDateTime')
                            # for el in items:
                            #    name_obj = el.getElementsByTagName(p_natrr + "name")[0]
                            if len(items) > 0:
                                for name_obj in items:
                                    ...
                                    for node in nodes:
                                        if node.nodeType == node.TEXT_NODE:
                                            p_reg_list['дата_размещения'] = node.data
                            if 'дата_размещения' not in p_reg_list:
                                items = pdoc.getElementsByTagName(p_ntag + 'ns9:startDT')
                                if len(items) > 0:
                                    for name_obj in items:
                                        nodes = name_obj.childNodes
                                        for node in nodes:
                            ...
                            if len(items) > 0:
                                for name_obj in items:
                                    ...
                                    break
                            if 'адрес_заказчика' not in p_reg_list:
                                items = pdoc.getElementsByTagName(p_ntag + 'postalAddress')
                                if len(items) > 0:
                                    for name_obj in items:
                                        nodes = name_obj.childNodes
                                        for node in nodes:
